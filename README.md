...
- **閲覧履歴**: 論文詳細ページの閲覧を自動記録、最近見た論文に素早くアクセス
- **バージョン管理**: 同一論文の arXiv 版 / 会議版などをバージョングループとして管理
...
- **タグ種別 (kind) システム**: タグを `venue` / `track` / `source` / `status` / `topic` の 5 種類に自動分類
- **クイックアクションボタン**: アイテム詳細ページに「既読にする」「自分の論文にする」ボタンを追加
- **検索タググループ化**: タグドロップダウンを `<optgroup>` で種別ごとに整理
- **会場ベース分析**: `/analytics` の集計をタグではなく `venue` フィールドで行うよう変更
- **ACL prefix バグ修正**: EMNLP/NAACL 等のトラックタグが `emnlp/findings` 等として正しく付与される
- **`ri tag migrate-kinds`**: 既存タグの kind を命名規則で一括再分類するコマンド

### v0.8 の新機能

- **論文一覧ページ**: `/papers` でライブラリ全体をページネーション付きで一覧表示
- **タイトルベースインポート**: タイトル指定で Semantic Scholar 経由のインポートが可能に
- **検索フィルタ強化**: 年・venue・タグ・著者名・種別のドロップダウンフィルタを追加
- **UI 日本語化**: Web UI の表示ラベルを日本語に統一
- **Chart.js 分析ダッシュボード**: `/analytics` グラフを Chart.js 4.x に全面リライト（出版年分布、登録推移、会場ランキング、種別内訳、著者ランキング）
- **Collection → Tag 統合**: コレクション機能をタグに置き換え

### v0.7 の新機能

- **閲覧履歴**: 論文詳細ページを開くと自動で履歴を記録、`/history` で最近見た論文50件を一覧表示
- **バージョン管理**: 同一論文の複数バージョン（arXiv v1、会議版など）をグループ化して表示
- **著者名フィルタ**: 検索ページに著者名による絞り込みを追加
- **引用グラフ改善**: DB 外参照（未解決の参照）を灰色ノードとして可視化、ワンクリックで DB に追加可能
- **PDF ブラウザ内表示**: PDF をダウンロードではなくブラウザ内で直接表示
- **ACL Anthology 改善**: IJCNLP/AACL のボリューム ID マッピングを修正
- **安定性向上**: S2 API の null 応答ハンドリング、Inbox テンプレートエラー修正
...
タグは命名規則で自動分類されます:
- `watch/*` → `source`（Watch 由来）
- `read` / `mine` / `to-read` など → `status`（ステータス）
- `acl` / `emnlp` / `iclr` など → `venue`（学会）
- `acl/main` / `emnlp/findings` など → `track`（トラック）
- その他 → `topic`
...
ページ一覧:
- **ホーム**: 統計、最近のアイテム
- **論文一覧 (`/papers`)**: ライブラリ全体のページネーション付き一覧（v0.8 新機能）
- **検索**: ハイブリッド検索（年、venue、タグ、著者名、タイプでフィルタ、タグは種別グループ表示）
- **アイテム詳細**: メタデータ、要旨、BibTeX、タグ（クイックボタン付き）、ノートエディタ、PDF閲覧、引用抽出、バージョン一覧
- **グラフ**: 引用サブグラフ可視化（D3.js、未解決参照の灰色ノード表示）
- **Inbox**: 発見した論文の承認/却下/自動承認
- **Watches**: ウォッチの管理・実行
- **ジョブ履歴**: パイプライン実行履歴
- **閲覧履歴**: 最近閲覧した論文の一覧（最新50件）
- **分析**: 出版年分布・登録推移・会場ランキング・著者ランキング・クラスタ・引用ネットワーク
